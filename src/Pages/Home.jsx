  ...
  return (
    <>
        <div className='text-white p-3  '>
            ...
            <ul className='my-5 p-5 lg:pl-10 bg-[#242323] drop-shadow-[0_3px_5px_rgba(0,0,0,.8)] rounded md:text-lg grid md:grid-cols-2  '>
                

                {TOPICS.map((topic, i)=>{
                    return <span key={i}  className='inline flex'>
                        <Link to={`/topic/`+topic.replace(/ /g,"-").toLowerCase()} className='hover:underline mb-3 p-1 flex items-center group' key={i} state={{title:topic}}>
